...
async function fetchFromApi(word: string): Promise<ApiEntry[] | null> {
  ...
  // The free Dictionary API is unofficial and community-run — it has no
  // uptime guarantee, and brief hiccups (a slow response, a momentary 5xx)
  // are common. A single failed attempt used to surface immediately as a
  // user-visible "network error" even when the API was fine a second
  // later. Two quick retries with a short backoff absorb most of that
  ...
  // Every attempt failed — this really is a network/service problem, not
  // a transient blip, so surface it as one.
  console.error(`Dictionary API failed after ${MAX_ATTEMPTS} attempts for "${word}":`, lastError);
