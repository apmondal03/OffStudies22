...
different kind of page from those two. Core 3000 and Advanced 1500 are
fixed, curated lists bundled into the app. This one has no fixed list at
all: it browses the same `cached_words` table the persistent word cache
already writes to, so it grows automatically, permanently, from real
usage — every word anyone looks up anywhere on the site becomes part of
it the first time.

- **A different architecture from the other two, on purpose.** Core 3000
  / Advanced 1500 are small enough to bundle client-side and filter
  entirely in the browser. This table has no fixed size and could grow
  indefinitely, so this page fetches from Supabase directly — a debounced
  search (`browseCachedWords` in `lib/dictionary/cachedWordsBrowse.ts`)
  with offset-based "Show more" pagination, rather than loading
  everything at once.
- **CEFR levels shown here aren't curated the way Core 3000's are** — a
  cached word gets its level from whatever the live dictionary API
  lookup produced (which defaults to B1 for anything not already in
  Core 3000), not from deliberate human assignment. Shown as the same
  badge for visual consistency, but worth knowing it's a looser signal
  here than on the other two pages.
- **Honest empty states, since this page will genuinely start at zero.**
  A fresh deployment has cached nothing yet — the page says so plainly
  ("try looking up a word first") rather than showing a blank grid or
  implying something's broken.

## Persistent word cache (`cached_words` table, `/admin/dictionary-cache`)

Every word fetched from the live modern dictionary API now gets saved to
Supabase — the next visitor who looks up the same word reads it straight
from our own database instead of hitting the external API again. This
directly reduces load on the free, rate-limited API this app depends on
for definitions, and compounds over time as real usage naturally builds
up a self-hosted dictionary.

- **`lib/dictionary/wordCache.ts`** — the Supabase read/write logic
  (`getCachedWord`, `saveCachedWord`, `isCacheStale`). Entries older than
  90 days are still served (better than nothing) but trigger a
  synchronous re-fetch rather than being trusted forever — a deliberate
  choice over a fire-and-forget background refresh, since serverless
  hosting gives no guarantee an unawaited background task survives after
  the response is sent, and a stale hit is rare enough (90 days) that a
  slightly slower response for that one request is the more reliable
  trade.
- **Kept in a genuinely separate file from the actual caching
  integration** — a real bug surfaced and got fixed during this build:
  the caching logic was first added directly inside
  `freeDictionaryProvider.ts`, but that file is shared by both server
  and browser code (its exported `getWord` branches on `typeof window`),
  and `wordCache.ts` needs `next/headers`, which can never legally
  appear in any client bundle. The production build caught this
  correctly and failed; a dynamic `import()` didn't actually fix it
  either (the bundler still traced it in). The real fix was structural:
  `lib/dictionary/cachedResolve.ts` wraps the caching logic and is only
  ever imported by genuinely server-only code — the `/api/word` Route
  Handler and the word page's Server Component — never by the shared
  dual-use file. Confirmed directly after the fix: no server-only code
  appears in any client-facing bundle chunk.
- **Admin-only, write-open table** — see the new "Cached dictionary
  words" section of `supabase/schema.sql` for the disclosed tradeoff:
  unlike every other table here, write access isn't restricted to signed
  in admins, because caching happens as a side effect of *any* visitor's
  word lookup, including anonymous ones.
- **`/admin/dictionary-cache`** shows how many words are cached and lets
  you download them as JSON (full structure) or CSV (one row per
  definition, spreadsheet-friendly). A second real bug was caught and
  fixed here too: the stats function originally threw an error when
  Supabase wasn't configured (checking "is this an admin" before
  checking "is Supabase even configured" — backwards from the order
  every other function in this codebase uses), which crashed both a
  build-time prerender attempt and, worse, the dev server itself on that
  route specifically. Fixed by checking configuration first and
  returning a quiet default, matching the established pattern elsewhere
  (e.g. `isRegistrationEnabled`) — never throwing over something that
  isn't really an authorization failure.

## Source attribution (word detail pages)

A real, disclosed licensing gap, now closed: nearly every word page's
content is Wiktionary-derived (only the 20 offline sample words aren't),
and Wiktionary is CC BY-SA — a license that requires attribution. Every
word page now credits Wiktionary and links the license at the bottom.
Showing this on the 20 sample-word pages too (which don't strictly need
it) is a harmless, deliberate simplification — there was no reliable
signal at render time to distinguish "this came from the offline
samples" from "this came from the live API" worth adding complexity for,
and over-attributing is a mild cosmetic choice, not a compliance risk,
unlike under-attributing.

## Full dictionary search index (`data/dictionary-index.json`)

Search autocomplete used to only suggest from Core 3000 + Advanced 1500
(~3,150 words) — typing anything outside that got no dropdown, even
though `/word/[slug]` could already look up any real word live. This
closes that gap with ~82,000 more searchable words, sourced from
Webster's Unabridged Dictionary (1913, public domain via Project
Gutenberg).

**Deliberately a search index only — no definitions stored from this
source.** Webster's 1913 prose is written for educated adults of that
era and is frequently *harder* to read than the word it's defining (the
entry for "abandon" cites 1913 marine insurance law). Storing that would
work against everything Core 3000 / Advanced 1500 / the modern API
integration already do to keep definitions learner-appropriate. Instead:
this file only makes a word *findable* by search — clicking a result
still flows into the exact same live modern-dictionary lookup every
other word already uses, in the exact same format, with real modern
definitions and synonyms. That live lookup is also the natural
"cross-check against current usage": a word this index suggests but the
modern API doesn't recognize just shows the existing graceful
not-found state, same as it would for any other word.

- **~11,300 entries Webster's own text marks `[Obs.]`/archaic were
  dropped** at import time, and everything already in Core 3000 /
  Advanced 1500 was deduplicated out (only ~82k of the raw ~113k
  headwords are genuinely new).
- **Server-only, ~1MB — never bundled to the browser.** Unlike Core 3000
  and Advanced 1500 (small enough to ship client-side), this is only
  ever read through a Server Action (`lib/dictionary/
  dictionarySearchAction.ts`) that returns a handful of matches per
  query, not the file itself. Confirmed directly in a build: the data
  appears only in the server bundle, and no client-side chunk grew.
- **`SearchBar` treats it as a lower-priority third tier** — Core 3000
  and Advanced 1500 results stay instant and synchronous, filling the
  suggestion list first; the big index is only consulted (debounced,
  200ms) when those don't already fill it, so normal typing never waits
  on a network round-trip.
- I could not build this by having Claude call the live modern
  dictionary API myself to pre-validate/enrich all ~113k source words —
  that API isn't reachable from this sandbox, and bulk-hammering a free,
  unofficial, volunteer-run service with 113,000 requests wouldn't be
  reasonable even if it were. The architecture above (index for search,
  live lookup for content, on-demand) achieves the same real-world
  outcome without needing to.
