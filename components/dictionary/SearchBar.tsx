...
import { searchFullDictionary } from "@/lib/dictionary/dictionarySearchAction";
...
interface SuggestionItem {
  ...
  partOfSpeech?: string;
  cefrLevel?: string;
  /** No cached metadata for this one — it comes from the ~82,000-word
   *  dictionary index, not Core 3000 / Advanced 1500, so it renders
   *  plainer (just the word) and its real definition is only fetched
   *  live once someone actually opens its word page. */
  plain?: boolean;
}
...
export function SearchBar({ size = "default", placeholder = "Search the dictionary…", autoFocus }: SearchBarProps) {
  ...
  // The big dictionary index only gets consulted once the fast, local,
  // curated lists don't already fill the suggestion list — debounced so
  // it never fires on every keystroke, and guarded against stale
  // responses arriving out of order for an old query.
  useEffect(() => {
    const q = query.trim();
    if (q.length < 2 || suggestions.length >= 8) {
      setExtraSuggestions([]);
      return;
    }
    ...
    const timer = setTimeout(async () => {
      const results = await searchFullDictionary(q, 8 - suggestions.length);
      if (requestIdRef.current === requestId) setExtraSuggestions(results);
    }, 200);
  ...
  const allSuggestions: SuggestionItem[] = useMemo(() => {
    const richWords = new Set(suggestions.map((s) => s.word.toLowerCase()));
    const extras = extraSuggestions
      .filter((w) => !richWords.has(w.toLowerCase()))
      .map((word) => ({ word, plain: true }));
  ...
  function handleKeyDown(e: React.KeyboardEvent<HTMLInputElement>) {
    const list = showRecent ? recent : allSuggestions.map((s) => s.word);
    if (e.key === "ArrowDown") {
      e.preventDefault();
      setOpen(true);
  ...
  return (
    <div ref={containerRef} className="relative w-full">
      ...
      {open && (showRecent || allSuggestions.length > 0) && (
        <ul
        ...
        >
          ...
          {!showRecent &&
            allSuggestions.map((s, i) => (
              <li key={s.slug ?? s.word} role="option" aria-selected={highlighted === i}>
                <button
                  type="button"
                ...
                >
                  <span>
                    ...
                    {s.partOfSpeech && <span className="ml-2 text-xs text-ink-faint italic">{s.partOfSpeech}</span>}
                  </span>
          ...
          {!showRecent && allSuggestions.length === 0 && query.trim() && (
            <li>
              <button
                type="button"
                onClick={() => goToWord(query)}
                className="flex w-full items-center gap-2.5 px-4 py-3 text-left text-sm hover:bg-surface-sunken"
              >
                <Search className="h-3.5 w-3.5 text-ink-faint" />
                <span>
                  Search the full dictionary for <span className="font-display">&ldquo;{query}&rdquo;</span>
                </span>
              </button>
            </li>
          )}
