...
import { importOwnWordsFromCsv, type OwnWordImportResult } from "@/lib/admin/ownDictionaryAdmin";
import { OWN_WORD_CSV_FIELDS } from "@/lib/admin/ownWordFields";
import { generateCsvTemplate } from "@/lib/admin/csv";

export function OwnWordCsvImportForm() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<OwnWordImportResult | null>(null);
  const [error, setError] = useState<string | null>(null);

  function downloadTemplate() {
    const csv = generateCsvTemplate(OWN_WORD_CSV_FIELDS);
    const blob = new Blob([csv], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "dictionary-words-template.csv";
    a.click();
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await importOwnWordsFromCsv(formData);
      setResult(res);
    } catch (err) {
    ...
    <div>
      <div className="rounded-2xl border border-border bg-surface p-5 mb-6">
        ...
        <p className="text-sm text-ink-muted mb-3">
          One column per field. List fields (&quot;examples&quot;, &quot;synonyms&quot;) use a pipe
          (<code className="font-mono">|</code>) to separate multiple items in one cell.
        </p>
        <button
          type="button"
        ...
        <div className="mt-5 flex flex-col gap-3">
          {result.succeeded > 0 && (
            <div className="flex items-center gap-2 rounded-xl border border-accent/40 bg-accent-soft px-4 py-3 text-sm text-accent-strong">
              ...
              {result.succeeded} {result.succeeded === 1 ? "word" : "words"} added successfully.
            </div>
          )}
